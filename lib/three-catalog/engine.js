...
class KMEngine {
  constructor() {
    ...
    this.SELECT_VIEWPORT_FRAC = 0.35;
  ...
  setSceneBackground(url) {
    ...
    loader.load(
      ...
      undefined,
      () => {
        if (target !== '/galaxybg0.png') this.setSceneBackground('/galaxybg0.png');
      }
    );
  }
  ...
  playTalkOnce() {
    // Talking disabled; keep idle loop running
    this._talking = false;
    if (this._talkCleanup) {
      clearTimeout(this._talkCleanup);
      this._talkCleanup = null;
    }
  }
  ...
  _updateTopStuffScale() {
    ...
    this.topStuff.scale.set(base.x * targetScale, base.y * targetScale, base.z * targetScale);
    this.topStuff.updateMatrixWorld(true);
  }
  ...
  init(container) {
    ...
    if (this._pendingBackgroundUrl) {
      ...
      this.setSceneBackground('/galaxybg0.png');
    }
  ...
  async loadProducts(items) {
    ...
    for (const it of items) {
      ...
      try {
        ...
        this.group.add(node);
        this.entries.set(it.id, { idx, root: node, baseQuat, pos: new THREE.Vector3(), targetSize, handle: it.handle, isSpecial: !!it.__special });
  ...
  _layoutEntries() {
    ...
    const bottomPadding = rowHeightPx;
  ...
  _computeSelectTargetVec() {
    ...
    const { h } = this._getCanvasSize();
    const frac = clamp(this.SELECT_VIEWPORT_FRAC, 0, 1);
    const screenY = frac * h;
    const worldYAtObjPlane = this._worldYAtObjFromScreenY(screenY);
    const worldY = this._mapYFromObjPlaneToZ(worldYAtObjPlane, this.SELECT_TARGET_Z);
    this._selectTargetVec.set(0, worldY, this.SELECT_TARGET_Z);
