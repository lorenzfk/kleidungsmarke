...
class KMEngine {
  ...
  async loadProducts(items) {
    ...
    for (const it of this.items) {
      ...
      this.entries.set(it.id, {
        ...
        targetSize: 1,
        gridScaleTarget: 1,
        selectedScaleTarget: null,
        returningScale: false,
        handle: it.handle,
        isSpecial: !!it.__special,
        failCount: 0,
        lastFailAt: 0,
      });
      idx++;
    ...
    this._layoutEntries();
    if (this._pruneSpecialOverlap()) {
      this._layoutEntries();
    }
    this._queueVisibleLoads(loadVersion);
    this._afterStable(() => { this._updateShelvesLayout(); this._layoutBarTop(); });
    setTimeout(() => {
      if (this._pruneSpecialOverlap()) {
        this._layoutEntries();
  ...
  _pruneSpecialOverlap() {
    if (!this.group) return false;
    const seen = new Set();
    const kept = new Map();
    let removed = false;
    for (const child of [...this.group.children]) {
      const id = child?.userData?.entryId;
      if (!id || !this.entries.has(id) || seen.has(id)) {
    ...
    for (const [id, entry] of this.entries) {
      const nextRoot = kept.get(id) || null;
      if (entry.root !== nextRoot) {
        entry.root = nextRoot;
        removed = true;
      }
    }
  ...
  _queueVisibleLoads(loadVersion = this._loadVersion) {
    if (!this.items || this.items.length === 0) return;
    if (!this.contentEl) return;
    if (loadVersion !== this._loadVersion) return;
    const metrics = this._computeGridMetrics();
    const columns = metrics.columns || 1;
    const rowHeightPx = metrics.rowHeightPx || 1;
    const topOffsetPx = metrics.topOffsetPx || 0;
    const viewportHeight = this.grid.viewportHeightPx || this._getCanvasSize().h || 0;
    const scrollTop = this.contentEl.scrollTop || 0;
    const bufferRows = 1;
    const startRow = Math.max(0, Math.floor((scrollTop - topOffsetPx) / rowHeightPx) - bufferRows);
    const endRow = Math.ceil((scrollTop + viewportHeight - topOffsetPx) / rowHeightPx) + bufferRows;
    for (let row = startRow; row <= endRow; row++) {
      for (let col = 0; col < columns; col++) {
        const idx = row * columns + col;
        if (idx < 0 || idx >= this.items.length) continue;
        const it = this.items[idx];
        if (!it || !it.id) continue;
        this._requestLoad(it.id, loadVersion);
      }
    }
    if (this.selectedId) this._requestLoad(this.selectedId, loadVersion);
    this._drainLoadQueue(loadVersion);
  }

  _requestLoad(id, loadVersion) {
    if (!id) return;
    const entry = this.entries.get(id);
    if (entry?.root) return;
    if (this._loadedIds.has(id) || this._loadingIds.has(id) || this._queuedIds.has(id)) return;
    this._queuedIds.add(id);
    this._loadQueue.push({ id, loadVersion });
  }

  _drainLoadQueue(loadVersion = this._loadVersion) {
    while (this._loadingIds.size < this._maxConcurrentLoads && this._loadQueue.length) {
      const next = this._loadQueue.shift();
      if (!next || next.loadVersion !== loadVersion) continue;
      this._queuedIds.delete(next.id);
      this._loadEntryById(next.id, loadVersion);
    }
  }

  async _loadEntryById(id, loadVersion) {
    ...
    try {
      ...
      entry.root = node;
      entry.baseQuat = node.quaternion.clone();
      entry.targetSize = targetSize;
      entry.gridScaleTarget = targetSize;

      this.group.add(node);
      this._loadedIds.add(id);
      this._layoutEntries();
      if (this._pruneSpecialOverlap()) this._layoutEntries();
      this._updateShelvesLayout();
      this._layoutBarTop();
    } catch (e) {
      console.error('GLB load error', it.modelUrl, e);
      entry.failCount = (entry.failCount || 0) + 1;
      entry.lastFailAt = Date.now();
      if (entry.failCount <= 3) {
        const delay = 400 * entry.failCount;
        if (this._retryTimers.has(id)) clearTimeout(this._retryTimers.get(id));
        const t = setTimeout(() => {
          this._retryTimers.delete(id);
          this._requestLoad(id, this._loadVersion);
          this._drainLoadQueue(this._loadVersion);
        }, delay);
        this._retryTimers.set(id, t);
      }
    } finally {
  ...
  _layoutEntries() {
    ...
    entries.forEach(([id, entry]) => {
      ...
      if (entry.root && !isSelected) entry.root.position.copy(entry.pos);
      ...
      if (entry.root && !isSelected && !entry.returningScale) {
  ...
  selectById(idOrNull) {
    ...
    if (this.selectedId) {
      ...
      this._requestLoad(this.selectedId, this._loadVersion);
      this._drainLoadQueue(this._loadVersion);
      if (e) {
