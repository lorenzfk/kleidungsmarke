...
class SoundController {
  ...
  async ensureRunning() {
    ...
    if (ctx.state === 'suspended') {
      ...
      if (ctx.state !== 'running') this._attachUnlockListeners();
    }
    if (ctx.state === 'running') this._detachUnlockListeners();
    return ctx;
  }
  ...
  async play(type) {
    ...
    gain.gain.setValueAtTime(preset.gain * MASTER_GAIN, now);
  ...
  _attachUnlockListeners() {
    if (typeof window === 'undefined' || this._unlockAttached) return;
    const resume = async () => {
      const ctx = await this.ensureRunning();
      if (!ctx) return;
      if (ctx.state === 'running') {
  ...
  _detachUnlockListeners() {
    if (!this._unlockAttached || !this._unlockHandler || typeof window === 'undefined') return;
    const resume = this._unlockHandler;
    ['pointerdown', 'touchstart', 'mousedown', 'keydown'].forEach((evt) =>
  ...
  _flushPending() {
    ...
    pending.forEach((type) => { this.play(type); });
  }
}
